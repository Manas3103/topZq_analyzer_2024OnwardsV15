#!/bin/bash
# Input arguments
input_dir=$1      # now a per-chunk filelist path, e.g. filelists/EGamma0_Era_H_Run24_chunk3.txt
output_file=$2    # Output ROOT file, e.g. EGamma0_Era_H_Run24_chunk3.root
...
echo "Input filelist : $input_dir"
echo "Output file    : $output_file"
echo "jobconfmod     : $jobconfmod"
echo "EOS Output Dir : $eos_output_dir"
...
# Sanity check: make sure the filelist actually made it to the worker node
if [ ! -f "${input_dir}" ]; then
...
# ./processnanoaod_new.py "$input_dir" "$local_output_path" "$jobconfmod" > "$log_file" 2>&1 || { echo "Error: Processing failed"; exit 1; }
